...
def PreProcess():
    ...
    return joints, positions, orientations,joint_file
...
def Camera2Base(joint, position, orientaion, file_name): #Frame Transform from camera to base
    #E -> End_effector
    #C -> Camera
    #B -> Base
    ...
    clamp_E = np.array([0,0.0102,0,1]) #along the z-axis(instead of y-axis in the guidence book)
    clamp_C = np.dot(Transfrom_E2B, clamp_E)
    print("Position of clamp in the base frame:\n", clamp_C)
    ...
    np.savetxt('./selected_frame_transform/' + file_name + '_position.txt', clamp_C, fmt='%.07f')

def main():
    joints,positions, orientations, file_name = PreProcess()
    for index in range(len(joints)):
        joint = joints[index][:-1]
        orientation = orientations[index]
        orientation.insert(0, orientation.pop())
        camera2base = Camera2Base(joint, positions[index], orientation, file_name[index][0:-4])
